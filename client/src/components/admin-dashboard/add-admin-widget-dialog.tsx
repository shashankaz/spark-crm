...
const CATEGORIES = ["All", "Tenants", "Revenue", "Users"] as const;
type Category = (typeof CATEGORIES)[number];

const CATEGORY_COLORS: Record<string, string> = {
  Tenants: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400",
  Revenue:
...
type AddAdminWidgetDialogProps = {
  activeWidgetIds: AdminCustomWidgetId[];
  onAdd: (widget: AdminWidgetMeta) => void;
};

export const AddAdminWidgetDialog = ({
  activeWidgetIds,
  onAdd,
}: AddAdminWidgetDialogProps) => {
  const [open, setOpen] = useState(false);
  const [selectedCategory, setSelectedCategory] = useState<Category>("All");

  const filtered = ADMIN_CUSTOM_WIDGETS.filter(
    (w) => selectedCategory === "All" || w.category === selectedCategory,
  );

  const handleAdd = (widget: AdminWidgetMeta) => {
    onAdd(widget);
    setOpen(false);
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose a chart widget to add to your admin dashboard
          </DialogDescription>
        </DialogHeader>
